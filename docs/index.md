---
title: Главная
description: Конспекты магистратуры «Управление и экономика здравоохранения» НИУ ВШЭ
hide:
  - navigation
  - toc
---

<section class="hero" markdown>

<span class="eyebrow">НИУ ВШЭ · магистратура</span>

# Управление и экономика здравоохранения

Студенческие конспекты магистратуры НИУ ВШЭ. Медицинские термины поясняем для немедиков, экономические и статистические — для врачей.

[Смотреть конспекты](year-1/notes.md){ .md-button .md-button--primary }
[Открыть 1 курс](year-1/index.md){ .md-button }
[Открыть программу ВШЭ](https://www.hse.ru/ma/health/courses){ .md-button }

</section>

## Материалы

<div class="feature-grid" markdown>

<article class="feature-card feature-card--accent" markdown>

### Конспекты

Конспекты по доказательной медицине, управлению здравоохранением, общественному выбору и микроэкономике.

[Открыть все конспекты →](year-1/notes.md)

[Доказательная медицина →](module-1/epidemiology-lecture-1.md)

[Управление изменениями в здравоохранении →](year-1/healthcare-management-and-change.md)

[Общественный выбор и бюрократия →](year-1/public-sector-economics-public-choice.md)

</article>

<article class="feature-card" markdown>

### Первый курс

Карты дисциплин, календарь модулей, формы контроля и ориентиры по курсовой.

[Смотреть структуру 1 курса →](year-1/index.md)

[Открыть памятку →](year-1/important.md)

</article>

</div>
